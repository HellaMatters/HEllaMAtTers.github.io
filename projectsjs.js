function openCat(evt, catName) {
    ...
    for (i = 0; i < tablinks.length; i++) {
        tablinks[i].className = tablinks[i].className.replace(" activeCat", "");
    }
    document.getElementById(catName).style.display = "block";
    evt.currentTarget.className += " activeCat";
...
bindEvents() {
    ...
    document.addEventListener("keyup", e => {
        if (this.gallery.classList.contains("is-visible")) {
            ...
            if (this.settings.keyboardNavigation) {
                if (e.key === 39) this.getNext();
                else if (e.key === 37) this.getPrevious();
            }
        }
    });
    ...
    this.gallery.addEventListener(
        "touchstart",
        e => {
            this.touch.startX = e.changedTouched[0].screenX;
        },
        false
    );
    ...
    this.gallery.addEventListener(
        "touchend",
        e => {
            this.touch.endX = e.changedTouched[0].screenX;
            this.handleGesure();
        },
        false
    );
...
var allTags = document.getElementById("main-content").getElementsByClassName("gallery");
